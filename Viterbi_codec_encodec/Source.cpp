...
Encodec_Viterbi::Encodec_Viterbi(vector<int> pols) {
	...
	kl++;
	max_l_res = pow(2, kl + 2);  // ограничение на кол-во путей держащих в памяти
}
Encodec_Viterbi::~Encodec_Viterbi() {
}
void Encodec_Viterbi::filling_matrix_weight() {
	...
	for (int i = 0; i < pow(2, kl); ++i) { // перебор всех вариантов регистра
		for (int j = 0; j < 2; ++j) { // перебор варинатов пришедших за шаг
			int u = i;
			u += j << kl;
			matrix_weight[i][u >> 1] = 0;
			for (int x = 0; x < polynom.size(); ++x) { //получение выходных бит
				char b = polynom[x] & u; // выход регистар
				int count = 0;
...
string Encodec_Viterbi::encodec(string in) {
	...
	for (int n_byts = 0; n_byts < input.size(); ++n_byts) {
		...
		// проверка на большое кол-во путей в памяти -> удаление пуетй с большим весом
